...
p1 <- plot.multinomial(combos)

## Convert to log scale to prevent underflow errors
post <- log(combos)
```

```{r fig.cap = "Figure 1. Distributions of historic ratings by attribute", fig.width = 6.5, fig.height = 4, fig.fullwidth = TRUE, echo = FALSE}
p1
```
...
```{r fig.cap = "Figure 2. Model accuracy as a function of number of samples", fig.width = 6.5, fig.height = 4, fig.fullwidth = TRUE, echo = FALSE}
p2
```